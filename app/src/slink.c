...
static enum SLINK_exit_code
event_loop(struct SLINK *s, bool has_screen) {
    SDL_Event event;
    while (SDL_WaitEvent(&event)) {
        switch (event.type) {
            case SC_EVENT_DEVICE_DISCONNECTED:
                ...
                return SLINK_EXIT_DISCONNECTED;
...
sc_audio_demuxer_on_ended(struct sc_demuxer *demuxer,
    ...
    const struct SLINK_options *options = userdata;
...
// Generate a SLINK id to differentiate multiple running SLINK instances
static uint32_t
SLINK_generate_scid(void) {
    struct sc_rand rand;
    sc_rand_init(&rand);
...
SLINK(struct SLINK_options *options) {
    static struct SLINK SLINK;
#ifndef NDEBUG
    // Detect missing initializations
    memset(&SLINK, 42, sizeof(SLINK));
#endif
    struct SLINK *s = &SLINK;
    ...
    enum SLINK_exit_code ret = SLINK_EXIT_FAILURE;
    ...
    uint32_t scid = SLINK_generate_scid();
    ...
    if (options->list) {
        ...
        ret = ok ? SLINK_EXIT_SUCCESS : SLINK_EXIT_FAILURE;
        goto end;
    }
...
aoa_complete:
    ...
    disconnected = ret == SLINK_EXIT_DISCONNECTED;
