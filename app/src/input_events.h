...
/* The representation of input events in SLINK is very close to the SDL API,
 * for simplicity.
 *
 * This SLINK input events API is designed to be consumed by input event
 * processors (sc_key_processor, sc_mouse_processor and sc_gamepad_processor,
 * see app/src/trait/).
 *
 * One major semantic difference between SDL input events and SLINK input
 ...
 * SLINK events are expressed in device frame coordinates.
 *
 * In particular, the window may be visually scaled or rotated, but this does
 * not impact SLINK input events (contrary to SDL input events). This allows to
 ...
 * implementation to convert from the SDL version to the SLINK version is
 * straightforward.
 *
 * In practice, there are 3 levels of input events:
 *  1. SDL input events (as received from SDL)
 *  2. SLINK input events (this API)
 ...
 * SLINK input event (2), then submitted to the relevant key/mouse processor,
