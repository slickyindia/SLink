...
#define SC_SERVER_FILENAME "SLINK-server"

#define SC_SERVER_PATH_DEFAULT PREFIX "/share/SLINK/" SC_SERVER_FILENAME
#define SC_DEVICE_SERVER_PATH "/data/local/tmp/SLINK-server.jar"

#define SC_ADB_PORT_DEFAULT 5555
#define SC_SOCKET_NAME_PREFIX "SLINK_"

static char *
get_server_path(void) {
    char *server_path = sc_get_env("SLINK_SERVER_PATH");
    if (server_path) {
        // if the envvar is set, use it
        LOGD("Using SLINK_SERVER_PATH: %s", server_path);
        return server_path;
    }
...
dbg = "-XjdwpProvider:adbconnection";
    ...
    cmd[count++] = "/"; // unused
    cmd[count++] = "com.slickyincorp.slink.Server";
    cmd[count++] = SLINK_VERSION;
...
run_server(void *data) {
    ...
    // exist, and SLINK will execute "adb connect").
...
sc_server_start(struct sc_server *server) {
    bool ok =
        sc_thread_create(&server->thread, run_server, "SLINK-server", server);
    if (!ok) {
