...
rescale_packet(struct sc_v4l2_sink *vs, AVPacket *packet) {
    AVStream *ostream = vs->format_ctx->streams[0];
    av_packet_rescale_ts(packet, SLINK_TIME_BASE, ostream->time_base);
}
...
sc_v4l2_sink_open(struct sc_v4l2_sink *vs, const AVCodecContext *ctx,
...
#ifdef SLINK_LAVF_HAS_AVFORMATCONTEXT_URL
    ...
    ok = sc_thread_create(&vs->thread, run_v4l2_sink, "SLINK-v4l2", vs);
    if (!ok) {
