...
static const AVRational SLINK_TIME_BASE = {1, 1000000}; // timestamps in us

static const AVOutputFormat *
find_muxer(const char *name) {
#ifdef SLINK_LAVF_HAS_NEW_MUXER_ITERATOR_API
    void *opaque = NULL;
#endif
...
sc_recorder_rescale_packet(AVStream *stream, AVPacket *packet) {
    av_packet_rescale_ts(packet, SLINK_TIME_BASE, stream->time_base);
}
...
sc_recorder_open_output_file(struct sc_recorder *recorder) {
    ...
    av_dict_set(&recorder->ctx->metadata, "comment",
                "Recorded by SLINK " SLINK_VERSION, 0);
...
sc_recorder_set_orientation(AVStream *stream, enum sc_orientation orientation) {
    ...
    uint8_t *raw_data;
#ifdef SLINK_LAVC_HAS_CODECPAR_CODEC_SIDEDATA
    AVPacketSideData *sd =
