...
#define SLINK_DEFAULT_ICON_DIR PREFIX "/share/icons/hicolor/256x256/apps"

static char *
get_icon_path(const char *filename) {
    char *icon_path;

    char *icon_dir = sc_get_env("SLINK_ICON_DIR");
    if (icon_dir) {
        ...
        LOGD("Using icon from SLINK_ICON_DIR: %s", icon_path);
        return icon_path;
    }

#ifndef PORTABLE
    icon_path = sc_file_build_path(SLINK_DEFAULT_ICON_DIR, filename);
    if (!icon_path) {
        LOG_OOM();
        return NULL;
    }
