...
`SLINK` execute `adb` commands to initialize the connection with the device. If
`adb` fails, then SLINK will not work.

This is typically not a bug in _SLINK_, but a problem in your environment.
...
case, SLINK fallbacks to a different method, which should work.
...
You could overwrite the `adb` binary in the other program, or ask _SLINK_ to
...
If _SLINK_ stops itself with the warning "Device disconnected", then the
...
On Windows, if `SLINK --otg` (or `--keyboard=aoa`/`--mouse=aoa`) results in:
...
[accented-characters]: https://blog.rom1v.com/2018/03/introducing-SLINK/#handle-accented-characters
[#37]:
...
On Plasma Desktop, compositor is disabled while _SLINK_ is running.
